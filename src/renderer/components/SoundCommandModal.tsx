...
interface SoundCommandModalProps {
  open: boolean;
  onClose: () => void;
  onSave: (data: SoundCommandUpsertInput) => Promise<void>;
  initialData?: SoundCommand | null;
  settings: SoundSettings;
}

type Step = 'content' | 'activation';

export function SoundCommandModal({ open, onClose, onSave, initialData, settings }: SoundCommandModalProps) {
  ...
  const pickFile = async () => {
    try {
      const selected = await window.copilot.pickSoundFile();
  ...
  const handleSave = async () => {
    ...
    if (commandEnabled) {
      ...
      if (!t.startsWith('!') || t.length < 2) {
        setError('Command must start with ! and include a name (e.g. !cat)');
        return;
      }
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-gray-900 border border-gray-700 rounded-xl w-full max-w-md shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700 shrink-0">
          <h3 className="font-semibold">{initialData ? 'Edit Sound Command' : 'New Sound Command'}</h3>
        ...
        <div className="flex items-center gap-2 px-5 pt-4 pb-1 shrink-0">
          ...
          <span className={`text-xs ${step === 'content' ? 'text-gray-200' : 'text-gray-400'}`}>Sound & Name</span>
        ...
        <div className="p-5 space-y-4 overflow-y-auto flex-1">
          {step === 'content' ? (
            <>
              <div>
                <label className="block text-sm text-gray-400 mb-1.5">Sound file <span className="text-violet-400">*</span></label>
                <div className="rounded-lg border border-dashed border-gray-600 bg-gray-800/40 p-5">
                  {filePath ? (
                    <div className="flex items-center gap-3 mb-3">
                      <span className="text-xl shrink-0">🎵</span>
                      <span className="text-sm text-gray-200 truncate flex-1">{getFileName(filePath)}</span>
                    </div>
                  ) : (
                    <p className="text-sm text-gray-500 text-center mb-3">No file selected yet</p>
                  )}
                  <div className="flex items-center justify-center gap-2">
                    <button type="button" onClick={() => void pickFile()} className="px-3 py-1.5 rounded bg-gray-700 hover:bg-gray-600 text-sm transition-colors">
                      {filePath ? 'Change file...' : 'Choose file...'}
                    </button>
                    {filePath ? (
                      <button type="button" onClick={() => void previewPlay()} className="px-3 py-1.5 rounded bg-gray-700 hover:bg-gray-600 text-sm transition-colors">
                        ▶ Preview
                      </button>
                    ) : null}
                  </div>
                  <p className="text-xs text-gray-600 text-center mt-2">Supported formats: MP3, OGG, WAV</p>
                </div>
              </div>
              <div>
              ...
              <div className={`rounded-lg border overflow-hidden transition-colors ${commandEnabled ? 'border-violet-600/50' : 'border-gray-700'}`}>
                ...
                >
                  <div className="text-left">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Plays when someone types a trigger in chat</p>
                  </div>
              ...
              <div className={`rounded-lg border overflow-hidden transition-colors ${scheduleEnabled ? 'border-cyan-600/50' : 'border-gray-700'}`}>
                ...
                >
                  <div className="text-left">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Plays automatically at regular intervals</p>
                  </div>
                  <ToggleSwitch checked={scheduleEnabled} onChange={setScheduleEnabled} />
                </button>
                {scheduleEnabled ? (
                  <div className="px-4 pb-4 pt-3 border-t border-gray-700 bg-gray-900/30">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
